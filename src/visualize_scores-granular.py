...
def apply_custom_styling(ax, fig, team1_name, team2_name, map_name, team1_color, team2_color, map_number, hill_duration, rotation_length):
    ...
    ax.axhline(y=0, color='white', linewidth=2)  # X-axis line at the bottom
    ax.axvline(x=0, color='white', linewidth=2)  # Y-axis line at the start
...
def visualize_cod_scores(
    ...
    if interval_points[-1] != len(df) - 1:
        interval_points = np.append(interval_points, len(df) - 1)
